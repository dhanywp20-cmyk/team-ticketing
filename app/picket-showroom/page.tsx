...
function exportToCSV(allRows:PiketRow[],kegiatanList:KegiatanEntry[]) {
  const headers=['No','Tanggal','Hari','PIC','Jenis Kegiatan','Jam Mulai','Jam Selesai','Produk','Tamu Instansi','Sales','Division','Kebutuhan','Keterangan'];
  const rows:string[][]=[];let no=1;
  allRows.forEach(piket=>{
    ...
    toR.forEach(kg=>{rows.push([String(no++),dateStr,piket.day_of_week,picName,kg?.jenis_kegiatan||'—',kg?.jam_mulai||'—',kg?.jam_selesai||'—',kg?.produk?.join(', ')||'—',kg?.tamu_instansi||'—',kg?.nama_sales||'—',kg?.sales_division||'—',kg?.kebutuhan?.join(', ')||'—',kg?.keterangan||'—']);});
  });
  const kgStats:Record<string,number>={};kegiatanList.forEach(k=>{kgStats[k.jenis_kegiatan]=(kgStats[k.jenis_kegiatan]||0)+1;});
  const csv=[headers.join(','),...rows.map(r=>r.map(c=>`"${c.replace(/"/g,'""')}"`).join(',')),'','STATISTIK JENIS KEGIATAN','Jenis,Jumlah',...Object.entries(kgStats).map(([k,v])=>`"${k}",${v}`)].join('\n');
  const blob=new Blob(['\ufeff'+csv],{type:'text/csv;charset=utf-8;'});
  const url=URL.createObjectURL(blob);const a=document.createElement('a');a.href=url;
  a.download=`Piket_Showroom_${new Date().toISOString().slice(0,10)}.csv`;a.click();URL.revokeObjectURL(url);
}
      ...
      <div className="relative z-10 flex flex-col min-h-screen">
        ...
        <header className="sticky top-0 z-50" style={{background:'rgba(255,255,255,0.9)',borderBottom:'3px solid #dc2626',backdropFilter:'blur(16px)'}}>
          <div className="max-w-[1600px] mx-auto px-6 py-3.5 flex items-center justify-between gap-4 flex-wrap">
            ...
            <div className="flex items-center gap-2">
              <button onClick={()=>exportToCSV(allRows,kegiatanList)}
